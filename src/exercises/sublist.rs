mod sublist {
    ...
    pub fn sublist<T: PartialEq>(_first_list: &[T], _second_list: &[T]) -> Comparison {
        let (sublist, superlist) = if _first_list.len() <= _second_list.len() {
        ...
        let superlist_len = superlist.len();
        let last_to_check = superlist_len - sublist_len;

        for start in 0..=last_to_check {
            let mut is_match_found = true;
            for index in 0..sublist_len {
                let sublist_val = sublist.get(index);
                let superlist_val = superlist.get(start + index);

                if superlist_val != sublist_val {
                    is_match_found = false;
                    break;
                }
            }
            if is_match_found {
                if superlist_len == sublist_len {
                    return Comparison::Equal;
                }

                if _first_list == superlist {
